...
| 表名 | 含义 | 关键字段 |
|------|------|----------|
| `st_rsvr_r` | 水库水情 | rz(水位), inq(入库), otq(出库), tm |
| `st_river_r` | 河道水情 | z(水位), q(流量) |
| `st_pptn_r` | 雨量 | p(雨量) |
| `st_pressure_r` | 渗压 | water_pressure, ext_pressure |
| `st_percolation_r` | 渗流 | percolation(渗流量) |
| `dsm_dfr_srvrds_srhrds` | GNSS位移 | wgs84_delta_h(位移量) |
| `rei_gate_r` | 闸门 | opening(开度), flow(流量) |
| `rei_pump_r` | 泵站 | current(电流), voltage(电压) |
| `wq_pcp_d` | 水质 | ph, do, nh3n, tn, tp |
| `st_soil_moisture_r` | 墒情 | moisture(湿度) |
| `st_termite_monitor_r` | 白蚁 | termite_count(蚁量) |
